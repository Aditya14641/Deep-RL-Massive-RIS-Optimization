...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, M, N, K, power_t, device, max_action=1):
        ...
        ideal_dim = 1 if state_dim == 0 else 2 ** (state_dim - 1).bit_length()
        hidden_dim = min(ideal_dim, 512) # Caps the hidden layer at 512 neurons
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        ideal_dim = 1 if (state_dim + action_dim) == 0 else 2 ** ((state_dim + action_dim) - 1).bit_length()
        hidden_dim = min(ideal_dim, 512) # Caps the hidden layer at 512 neurons
